...
# Backs up supply chain JSON and registry on any change.
#
...
ENV_FILE="REDACTED/.env"
LOG_FILE="REDACTED/logs/registry-sync.log"
...
REGISTRY_URL="http://localhost:${REGISTRY_PORT:REDACTED}"
...
backup_triggered=0
backup() {
    if [[ $backup_triggered -eq 1 ]]; then return; fi
    backup_triggered=1
...
generate_sbom() {
    ...
    if "$SYFT" "registry:localhost:${REGISTRY_PORT:REDACTED}/$image" -o json > "$sbom_path" 2>> "$LOG_FILE"; then
...
" 2>/dev/null || true)
    ...
    while IFS= read -r TAG; do
        ...
        MANIFEST_RESPONSE=$("$CURL" -sf \
            -H "Accept: application/vnd.docker.distribution.manifest.v2+json" \
            -D - \
            "$REGISTRY_URL/v2/$REPO/manifests/$TAG" 2>> "$LOG_FILE" || echo "")

        REGISTRY_DIGEST=$(echo "$MANIFEST_RESPONSE" | grep -i "^docker-content-digest:" | tr -d '[:space:]' | cut -d: -f2- | tr -d '\r' || echo "unknown")

        if [[ -z "$REGISTRY_DIGEST" || "$REGISTRY_DIGEST" == "unknown" ]]; then
...
data['entries'].append({
    ...
    'type':            'docker-image',
    'source':          '',
    'local_tag':       'localhost:${REGISTRY_PORT:REDACTED}/$IMAGE',
...
PYEOF
            ...
            # --- Existing entry ---
            STORED_DIGEST=$("$PYTHON" -c "import json,sys; print(json.loads('''$EXISTING''').get('digest',''))")
            STORED_SBOM=$("$PYTHON" -c "import json,sys; print(json.loads('''$EXISTING''').get('sbom_path',''))")
            ...
            if [[ $DIGEST_MATCH -eq 0 || $SBOM_OK -eq 0 ]]; then
                backup
                CHANGES=$((CHANGES + 1))
...
with open('$SUPPLY_CHAIN_FILE', 'w') as f:
    json.dump(data, f, indent=2)
PYEOF
                log "INFO" "UPDATED: $IMAGE — entry and SBOM refreshed"
